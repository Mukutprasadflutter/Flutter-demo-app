...
class Landing extends State<LandingScreen> {
  var feeds = new List<FavouriteModel>();
  int _currentIndex = 0;
  ...
  Widget _getItemUI(BuildContext context, FavouriteModel feedModel) {
  ...
  _makeGetRequest() async {
    ...
    if (statusCode == 200) {
      setState(() {
        ...
        feeds = list.map((model) => FavouriteModel.fromJson(model)).toList();
      });
    }
  }

  void postLike(FavouriteModel feedModel, BuildContext context) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? "";
  ...
  void postUnlike(FavouriteModel feedModel, BuildContext context) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token') ?? "";
    Map<String, String> headers = {
      "Content-type": "application/json",
  ...
  uploadImage(File imageFile, int recipeId, FavouriteModel feedModel) async {
    var stream =
  ...
  //=========================================SNACK BAR===========================================
  _showSnackBar(BuildContext context, FavouriteModel item) {
  ...
  //=========================================Click Events========================================
  void openDetailScreen(FavouriteModel feedModel, BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  void getImage(int recipeId, FavouriteModel feedModel) async {
